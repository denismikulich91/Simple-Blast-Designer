...
class Canvas(wx.Panel):
    ...
    def __init__(self, parent, properties_panel, layer_panel):
        ...
        # self.main_canvas.Bind(FloatCanvas.EVT_FC_LEFT_DOWN, self.RectGotHitLeft)
        # for object in self.all_objects_dict[self.layer_manager_panel.get_active_layer]:
        #     object.Bind(FloatCanvas.EVT_FC_LEFT_DOWN, self.RectGotHitLeft)

    def RectGotHitLeft(self, evt, object_id, single_object):
        print(object_id)
        self.properties.set_property_table(object_id, self.lines_and_points.lines_dict[object_id])
        print(self.lines_and_points.lines_dict[object_id]['coordinates'])
