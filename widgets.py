    ...
    def __init__(self, label, tag):
        ...
        with dpg.window(label=self.label, width=500, height=-1, pos=(300, 300), tag=self.tag):
            with dpg.group():
                ...
                with dpg.group(horizontal=True):
                    dpg.add_slider_int(min_value=1, max_value=10, width=300, callback=self.update_sketch_polygon, tag='string_width_slider')
                ...
                with dpg.group(horizontal=True):
                    ...
                    with dpg.group():
                        with dpg.drawlist(width=300, height=100, tag='sketch_list'):
                            dpg.draw_polygon(self.sketchCoordinates, color=self.sketch_color, thickness=self.sketch_line_width, tag='sketch_polygon')
...
class AppButtons:
    ...
    def import_from_surpac():
        ImportSettings('Import from Surpac', 'surpac_import_window')
